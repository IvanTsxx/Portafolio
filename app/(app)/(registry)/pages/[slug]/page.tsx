...
import { getPage, getPages } from "@/shared/lib/registry";
...
export async function generateStaticParams() {
  return getPages().map((b) => ({ slug: b.name }));
}
...
export default async function PagePage({ params }: Props) {
  ...
  return (
    <div className="mx-auto bg-background max-w-3xl px-4 py-16">
      ...
      {/* File tree — show all files in the page */}
            ...
            {entry.files.map((file, i) => (
              ...
              >
                <span className="  text-[12px] text-muted-foreground">
                  {file.type === "registry:page" ? "page" : "file"}
                </span>
